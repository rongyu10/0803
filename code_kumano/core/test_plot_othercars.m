function ems = test_plot_othercars(othercars, SIMPLECARSHAPE, REALCARSHAPE,idx_frontL,idx_frontR,idx_rearL,idx_rearR)
...
if fist_flag
    ...
    %for i = 1:othercars.MAX_NRCAR
        ...
        if SIMPLECARSHAPE
            if isfield(othercars.car{i},'IDM')&&isfield(othercars.car{i}.IDM,'polite')&&(othercars.car{i}.IDM.polite == 1)
                h.carsfill{i} = fill(othercars.car{i}.bd(:, 1), othercars.car{i}.bd(:, 2), 'c');
            else
    ...
    %for i = 1:othercars.MAX_NRCAR
        if SIMPLECARSHAPE
            ...
            if isfield(othercars.car{i},'IDM')&&isfield(othercars.car{i}.IDM,'polite')&&isfield(othercars.car{i}.IDM,'angry')
               if othercars.car{i}.IDM.angry ==1
                   h.carsfill{i}.FaceColor = 'r';
               elseif othercars.car{i}.IDM.angry ==2
                   h.carsfill{i}.FaceColor = 'k';
               else
                   if othercars.car{i}.IDM.polite == 1
                       h.carsfill{i}.FaceColor = 'c';
                   else
                       h.carsfill{i}.FaceColor = col;
                   end
               end            
            end      
            
            %if (i == idx_frontL)||(i == idx_frontR)||(i == idx_rearL)||(i == idx_rearR)
            if (i == idx_rearL)
                h.carsfill{i}.FaceColor = 'y';
            elseif (i == idx_rearR)
                h.carsfill{i}.FaceColor = 'g';
            elseif (i == idx_frontL)
                h.carsfill{i}.FaceColor = 'b';
            elseif (i == idx_frontR)
                h.carsfill{i}.FaceColor = 'w';
            else
                %h.carsfill{i}.FaceColor = col;
            end
