...
%-----------------------------
%-- Thicker Boudary in tollplaza
tmp = road.track{2}.seg{1}.bd;
plaza_data  = tmp(3:end-1,:);
isize_plaza = size(plaza_data,1);
for i=1:isize_plaza
    road_data=[road_data;plaza_data(i,1) plaza_data(i,2)+ 2.5*10^3];
    road_data=[road_data;plaza_data(i,1) plaza_data(i,2)+ 5*10^3];
    road_data=[road_data;plaza_data(i,1) plaza_data(i,2)+ 7.5*10^3];    
end
...
while sim.flag && ishandle(fig)
    ...
    switch sim.mode 
        case 'RUN'
            ...
            %--- Resetting Goal position for waiting line of gate--------
            goal = reset_goal_position(road.track{4}, GOAL_GATE, mycar, othercars);
            %------------------------------------------------------------
...
%                 % plot obstacles
                  if sim.tick==2
                    ...
                    f2 = scatter(goal(1),goal(2),120,'MarkerEdgeColor','k','MarkerFaceColor','y','LineWidth',1.5);
                  elseif sim.tick>2
                    ...
                    if mycar.pos(1)< 230*10^3
                       %evalParam = [250.0,50.0,60.0,2.3];  % original [20.0,230.0,60.0,2.0]
                       evalParam = [120.0,20.0,60.0,2.4];
                    else
