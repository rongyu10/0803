...
while sim.flag && ishandle(fig)
    ...
    switch sim.mode 
        case 'RUN'
            ...
            if FLAG_methodDWA ==5
                [risk_rearL,risk_rearR] = predict_risk_of_followingCars(mycar, othercars);
            end
            %----   DWA --------------------------------------------------
                  obstacle  =[];
                  for i=1:othercars.n
...
%                   end
                    ...
                    if mycar.pos(1)< 230*10^3
                       evalParam = [20.0,230.0,50.0,2.0];  % [40.0,200.0,20.0,2.0]
                    else
            ...
            if FLAG_DynamicWindow
                 if FLAG_methodDWA ==5
                    [mycar,traj_DWA, eval_DWA] = Control_by_DynamicWindowApproach(mycar,othercars, sim, goal,all_obstacle,obstacleR, Kinematic, evalParam, area, PLOT_DWA, FLAG_methodDWA, risk_rearL,risk_rearR);                 
                 else
