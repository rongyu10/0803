...
% LANE CHANGE SETTING--------
FLAG_LANECHANGE = 0;
dx = 30000/3; % x-cood.interval of control points for bezier curve
ctlPt = [0 0; dx 0; 2*dx -LANE_WIDTH; 3*dx -LANE_WIDTH];
[laneChangePath, lengthP] = bezierCurve(ctlPt);
ratioSpeed = lengthP/30000*1.1;
...
Kinematic = [23000, (40.0)/180*pi, 5000, (40.0)/180*pi,200, (1)/180*pi];
%評価関数のパラメータ [heading,dist,velocity,predictDT]
evalParam = [5.0,50.0,5.0,2.2];  % [0.1,0.45,0.1,4.0]
...
while sim.flag && ishandle(fig)
    ...
    switch key_pressed 
        ...
        case 'c'
            ...
            othercars.car{idxcar}.accele = -2000/sim.T;
        case 'v'
            % Checking for Intelligetn Driver Model (add by kumano)
            idxcar = ceil((othercars.n)/2);
            othercars.car{(othercars.n)-1}.vel(1) = othercars.car{(othercars.n)-1}.vel(1) - 2000;
            othercars.car{(othercars.n)-1}.accele = -2000/sim.T;
        case 'z'
            % Checking for Intelligetn Driver Model (add by kumano)
    ...
    switch sim.mode 
        case 'RUN'
            ...
            if FLAG_LANECHANGE~=1
                flag_congested = false;
                [FLAG_LANECHANGE, data_laneChange] = judge_lanechange_action_HighSpeed(mycar, myinfo, othercars, sim, data_laneChange, flag_congested);
                mycar = negotiate_othercars_HighSpeed(mycar, othercars, sim, flag_congested);
            end
...
%                   end
    ...
    strtemp = ['[VEL: %.1fKM/H %.1fDEG/S] \n'];    
    titlestr = sprintf(strtemp, mycar.vel(1)/10000*36, mycar.vel(2));
    %-----------------------------------
