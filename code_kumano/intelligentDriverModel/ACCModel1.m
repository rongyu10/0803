function othercars = ACCModel1(othercars,mycar,myinfo,road,sim,varargin)
...
v0_gate          = 7000;     % 5000 desired_velocity[mm/s]
v0_gate_exitSide = 10000;    % 5000 desired_velocity[mm/s]

%---- Enter-side/ Exixt-side of Toll Plaza -------
%----- Added OFF RAMP condition
tollGateFlag = true;
if (nargin == 5)
   enterFlag = true;
elseif (nargin == 6)&&(strcmp(varargin{1},'enter'))
   enterFlag = true;
elseif (nargin == 7)&&(strcmp(varargin{1},'enter'))
   enterFlag = true;
   v0_gate   = varargin{2};
elseif (nargin == 6)&&(strcmp(varargin{1},'exit'))
   enterFlag = false; 
elseif (nargin == 7)&&(strcmp(varargin{1},'exit'))
   v0_gate_exitSide = varargin{2};
elseif (nargin == 6)&&(strcmp(varargin{1},'offramp'))
   tollGateFlag = false;
end
%--------------------------------------------------
    ...
    if idx ~= idx_mycar
       ...
       theta = othercars.car{idx}.pos(3);
          ...
          if iforward == 0
             ...
             %deltaV  = v - v_1;
             %--- Leading car case (must be checked!!) -----
             v_1 = v0_norm;
             deltaV  = v - v_1;
             s_alpha = 500*10^3;
             %----------------------------------------------
          ...
          %----Speed Limit at the gate (Enter/Exit-side)--
          if tollGateFlag
          ...
          if (x(1) > 0) % update speed for cars inside the simulator road
            ...
            othercars.car{idx}.accele = accele_ACC;
...
allBD_mat = get_allBoundary_in_matrix(othercars, mycar, xorder, mycar_idx); % all cars boudary data for "get_forwardCarIndex_modified2"
...
%        end
       ...
       %-- get index of forward car--------
       %iforward = get_forwardCarIndex(i, xorder, allBD, searchbd);
       %iforward  = get_forwardCarIndex_modified(i, xorder, allBD, searchbd); 
       iforward  = get_forwardCarIndex_modified2(i, allBD_mat, searchbd);     % Fastest(2017/11/13)
...
%---- find points(otherBDall) inside the boundary(searchbd)---
...
function iforward = get_forwardCarIndex_modified2(i_mynum, allBD_mat, searchbd)
%  THE FASTEST version of get_forwardCarIndex(2017/11/13)
...
if sum(P)==0
    ...
    idx_inside = find(P,1,'last');     % use "last" to find car with minimum x coordinate(closest car) 
    iforward  = ceil(idx_inside/NSET); % get car ID ( car1: 1-7, car2: 8-14,car3: 15-21.....) 
end
%--------
...
function allBD_mat = get_allBoundary_in_matrix(othercars, mycar, xorder, mycar_idx)
%-- number of data set of each car ---
NSET = 7;
%-------------------------------------
nr_cars    = length(xorder);
allBD_mat  = zeros(nr_cars*NSET,2);

%--- othercars positions-
for i=1:nr_cars
    if i == mycar_idx
      tmp_BD = get_carLineData(mycar);        
    else
      idx = xorder(i);
      tmp_BD = get_carLineData(othercars.car{idx});
    end
    ista     = (i-1)*NSET +1;
    iend     = i*NSET;
    allBD_mat(ista:iend,:) = tmp_BD';
end
%-------------------------
...
if isempty(idx_x) 
   %error('Error: ACCModel1(current_indexX) ')
   idx_x = n_data;
end
...
%{
function idx_x = current_indexX(pos,routeData)
% get current index on trajectory data based on X coordinate(SLOW...) 
data    = routeData.route;
n_data  = size(data,1);
curr_x  = pos(1);
