function ems = plot_mycar(mycar ...
    , PLOT_FUTURE_CARPOSES, PLOT_CAR_PATHS, SIMPLECARSHAPE,REALCARSHAPE, PLOT_RFS)
% PLOT MYCAR
...
if first_flag
    ...
    if SIMPLECARSHAPE
        ...
        [h1, h2, h3] = plot_arrow(mycar.pos, 2000, mycar.pos(3), 'k', 2);
        h.cararrow = [h1 h2 h3];
    end
    ...
    if SIMPLECARSHAPE
        ...
        [x1, y1, x2, y2, x3, y3] = get_arrow(mycar.pos, 2000, mycar.pos(3), 'k', 2);
        ...
        % for lane change judgement
        
        if isfield(mycar,'lanechange')&&(mycar.lanechange==1)
            h.carfill.FaceColor = 'b';
        else
            h.carfill.FaceColor = 'g';
        end    
    end
    % CAR IMAGE (PLOT ALWAYS)
    if 1
