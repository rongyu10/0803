function mycar = calculate_velocity_mycar_tollPlaza_IN_1123_judge(mycar, sim, othercars, idm, laneChangePath, MYCAR_AGGRESSIVE_MODE)

persistent idx_precedingcar;
...
if mycar.pos(1) > mycar.x_start_detecting
    ...
    if isempty(idx_precedingcar)
        [idx_crossingcar, arr_t_mycar, arr_t_othercar, dist_section_mycar, rel_deg_crossingcar, sidepoint, invadepoint] = detect_crossing_car_1123(othercars, mycar);
    end
end

if ~isempty(idx_crossingcar)
    if abs(arr_t_othercar - arr_t_mycar) > mycar.max_time_dif_intersection % if over 1(sec) difference between two cars, mycar does not accelerate and decelerate
...
if ~isempty(idx_precedingcar)
    ...
    temp_acceleration = calculate_acceleration_IDM(mycar, othercars.car{idx_precedingcar}, idm, rel_deg_precedingcar);
...
if ~isempty(idx_precedingcar)
    if mycar.acceleration < -2940
        fprintf(2, 'mycar([%d, %d]) decelerate([%d]) to car [%d] (reldegree = [%d])\n', mycar.pos(1), mycar.pos(2), mycar.acceleration, idx_precedingcar, rel_deg_precedingcar);
    else
